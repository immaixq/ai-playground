...
from models.supcon_clip import SupConClipModel, unfreeze_visual_encoder_layers, contrastive_loss
...
def main():
    ...
    try:
        ...
        try: 
            ...
            try: encoder_output_dim = visual_model.ln_post.normalized_shape[0]
            except AttributeError: encoder_output_dim = 512; logger.warning("VERIFY encoder_output_dim!")
        
        logger.info(f"CLIP model loaded. Encoder output dim: {encoder_output_dim}")
    except Exception as e: logger.error(f"Failed to load CLIP: {e}", exc_info=True); return
    ...
    try:
        train_paths, val_paths = split_data(config.IMG_DIR, config.VALIDATION_SPLIT, config.SEED)
        if not train_paths: raise ValueError("No training data.")
    ...
    except Exception as e: logger.error(f"Data prep failed: {e}", exc_info=True); return

    params_to_train = list(filter(lambda p: p.requires_grad, model.parameters())) # Filter on combined model
    if not params_to_train: logger.error("No trainable parameters!"); return
    ...
    for epoch in range(config.EPOCHS):
        train_pbar = tqdm(train_loader, desc=f"Epoch {epoch + 1}/{config.EPOCHS}", leave=False)
        for batch in train_pbar:
            if batch is None: continue
            images, labels = batch # Single-view data
            ...
            try:
                ...
                # Backward, Clip, Step
                loss.backward()
                if config.GRAD_CLIP_NORM > 0: torch.nn.utils.clip_grad_norm_(params_to_train, config.GRAD_CLIP_NORM)
                optimizer.step()
                scheduler.step()

            except Exception as e: logger.error(f"Training error: {e}", exc_info=True) ; continue
